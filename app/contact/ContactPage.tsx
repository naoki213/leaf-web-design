...
export default function ContactPage(){
 ...
 return <main className="contact-page">
  ...
  <section className="contact-intro"><p className="contact-index">お問い合わせ</p><div><span>無料相談</span><h1>まだ言葉にできない、<br/>その段階から。</h1><p>つくりたいページ数や、載せたい内容が決まっていなくても大丈夫です。今のお仕事のことから、ゆっくりお聞かせください。</p></div></section>
  <section className="contact-details"><div className="contact-notes"><p>ご相談の前に</p><dl><div><dt>制作料</dt><dd>0円（税込）</dd></div><div><dt>公開後</dt><dd>月額2,000円（税込）</dd></div><div><dt>制作規模</dt><dd>10〜20ページ対応</dd></div><div><dt>別途費用</dt><dd>サーバー代・ドメイン代</dd></div></dl><small>相談したからといって、契約する必要はありません。</small></div>
   <form onSubmit={onSubmit}><div className="field"><label htmlFor="name">お名前</label><input id="name" name="name" autoComplete="name" required/></div><div className="field"><label htmlFor="email">メールアドレス</label><input id="email" name="email" type="email" autoComplete="email" required/></div><div className="field"><label htmlFor="business">お店・事業について</label><input id="business" name="business" placeholder="例：美容室、飲食店、設計事務所"/></div><div className="field"><label htmlFor="url">現在のWebサイト</label><input id="url" name="url" type="url" inputMode="url" placeholder="お持ちの場合のみ"/></div><div className="field"><label htmlFor="message">ご相談内容</label><textarea id="message" name="message" rows={7} placeholder="決まっている範囲でご記入ください" required/></div><button type="submit">入力内容を確認する <span>→</span></button>{confirmed&&<p className="form-notice" role="status">入力内容を確認しました。現在、送信先を準備しています。正式な送信機能の設定前に入力内容が外部へ送られることはありません。</p>}</form>
  </section>
